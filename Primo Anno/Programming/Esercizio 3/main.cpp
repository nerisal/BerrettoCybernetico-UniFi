...
int main() {
    ...
    bool useKnight = true;
    if (useKnight) {
        // Posso farlo perché sto puntando a degli oggetti di tipo derivato.
        // Per il principio di sostituzione di Liskov, posso puntare a Knight.
        hero = new Knight(*"Artù");
    }else {
        hero = new Wizard(*"Merlino");
    }
    ...
    Weapon* weapon;
    if (useSword) {
        weapon = new Sword(10, false, false);
    }else {
        weapon = new Bow(10, 15);
    }
    hero->setWeapon(weapon);
    // create an enemy (an Orc or a Skeleton)
    GameCharacter* enemy;
    GameCharacter* orc = new Orc(10);
