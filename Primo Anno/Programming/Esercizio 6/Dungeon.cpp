...
void Dungeon::saveToFile(std::string fileName) throw(GameFileException) {
    ...
    try {
        ...
        std::cerr << e.what();
        // DONE throw GameFileException, non fatal
        throw GameFileException("Can not read map file", fileName, false);
...
void Dungeon::loadFromFile(std::string fileName) throw(GameFileException, std::runtime_error, std::out_of_range) {
    ...
    try {
        ...
        // DONE throw GameFileException, fatal
    ...
    if (fileLine.compare(MAP_FILE_SIGNATURE) != 0)
        throw GameFileException("File is not a map", fileName, true);
        // DONE throw GameFileException, fatal
    std::getline(in, fileLine);
    ...
    while(!in.eof() && std::getline(in, fileLine)) {
        for (int x = 0; x < xsize; x++) {
            try {
                ...
                throw GameFileException("Can not set map title at x: " + std::to_string(x) + "- y", fileName, true);
                // DONE throw out_of_range with message "Can not set map tile at x: " + std::to_string(x)+" - y: " + std::to_string(y)
            }
        }
        y++;
    }
    if (y!=ysize)
        // DONE throw runtime_error with message: "Map height size wrong. Expected "+std::to_string(ysize)+" but read: " + std::to_string(y)
            throw GameFileException("Map height size wrong. Expected "+std::to_string(ysize)+" but read: " + std::to_string(y), fileName, true);
    in.close();
}
