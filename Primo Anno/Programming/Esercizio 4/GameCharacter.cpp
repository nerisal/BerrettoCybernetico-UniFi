...
GameCharacter::GameCharacter(int hp, int a)
    : HP(hp), armor(a), posX(0), posY(0), weapon(nullptr), movements(1) {}
...
GameCharacter::GameCharacter(const GameCharacter &original) {
  ...
  if (original.weapon != nullptr)
    weapon = original.weapon;  // DONE: use Weapon clone()
...
GameCharacter &GameCharacter::operator=(const GameCharacter &right) {
  if (this != &right) {
    ...
    if (right.weapon != nullptr)
      weapon = right.weapon;  // DONE: use Weapon clone()
    else
      weapon = nullptr;
  }
  return *this;
...
void GameCharacter::setHP(int hp) { this->HP = hp; }

Weapon *GameCharacter::getWeapon() { return weapon; }

void GameCharacter::setWeapon(Weapon *weapon) { this->weapon = weapon; }
...
int GameCharacter::getArmor() const { return armor; }

void GameCharacter::setArmor(int defense) { this->armor = defense; }

int GameCharacter::basicFight(GameCharacter &enemy) {
  Dice myDice(6);

  int hit = 1;
  if (weapon) hit = weapon->use();
...
bool GameCharacter::isLegalFight(const GameCharacter &enemy) const {
  int maxDistance = 1;
  if ((abs(posX - enemy.posX) > maxDistance) ||
      (abs(posY - enemy.posY) > maxDistance))
    return false;
  return true;
}
...
int GameCharacter::getMovements() const { return movements; }
